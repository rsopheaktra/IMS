...
class Employees
{
     ...
     */
     ...
     function CreateTable(){
          $stmt = $this->con->prepare("CREATE TABLE IF NOT EXISTS `Employees` (`id` INT NOT NULL AUTO_INCREMENT, `first_name` VARCHAR(100), `last_name` VARCHAR(100), `telephone` VARCHAR(50), `email` VARCHAR(50), `address` VARCHAR(255), `zipcode` VARCHAR(50), `city` VARCHAR(50), `country` VARCHAR(255), `departement_id` INT, PRIMARY KEY (`id`)); ");
          if($stmt->execute()) {
             return true; 
          }
          return false;
     ...
     */
     function DataEntry($images, $name, $type, $address, $zipcode, $city, $description){
          $stmt = $this->con->prepare("INSERT INTO entreprise(images, name, type, address, zipcode, city, description) VALUES (?, ?, ?, ?, ?, ?, ?)");
          $stmt->bind_param("sssssss", $images, $name, $type, $address, $zipcode, $city, $description);
          if($stmt->execute()) {
             return true; 
          }
          return false; 
